...
2. **`ARCH-19`**, **`U-13`** — 2 inconsistencias de comportamiento
   descubiertas incidentalmente cerrando `ARCH-16` (no son hallazgos de
   linting, quedan para revisión de producto)
3. **`U-12`** 🟡 — Modo oscuro, en progreso por fases — ver detalle abajo

`ARCH-15`, `ARCH-16`, `ARCH-17`, `ARCH-18`, `SEC-14`, `SEC-15` y `U-11` ✅
cerrados — ver tablas de Arquitectura y Seguridad abajo.
...
| **U-12** 🟡 | Sin soporte de `prefers-color-scheme` (modo oscuro) — preferencia de producto, no defecto (auditoría QA del 12 de julio, segunda pasada) | tokens `--color-*`/`--brand-*` en `src/index.css`, 30 archivos `.css` de componentes | 🟡 **En progreso por fases (13 de julio)** — alcance real, verificado por `grep` antes de empezar: ~130 colores hex únicos, 526 apariciones, en 30 archivos (más grande de lo que sugería la descripción original). **Fase 1 (completa):** se extendió `:root` con 11 tokens nuevos para los valores más repetidos que no tenían token (`--color-text-muted`, `--color-success-mid/strong/darker/bg-alt/light/text`, `--color-danger-lighter/text`, `--color-role-coord-bg`, `--color-warning-dark`) — mismo criterio que `FE-3`/`FE-5`, solo se nombra lo que ya estaba en uso, los colores de un solo archivo se dejan literales. Se agregó el bloque `@media (prefers-color-scheme: dark)` redefiniendo todos los tokens de color, reusando la paleta `--navy-950/900/800/700` que ya existe y está probada en producción (sidebar) para las superficies, en vez de inventar colores nuevos. 100% aditivo, no toca ningún componente. **Fase 2 (iniciada):** migración archivo por archivo de hex literal a `var()`. `DocenteScan.css` (29 valores) migrado como caso piloto — verificado que cada `var()` usado resuelve contra un token real de `:root` (`comm` contra la lista de tokens definidos), `vite build` limpio, 153/153 tests. Quedan **28 archivos** (`UploadPreviewModal.css` 93, `AdminQRPanel.css` 80, `ReporteAsistencias/index.css` 77, `HistorialView.css` 44, `ResumenView.css` 34, `LogsView.css` 26, `QRProyeccion.css` 21, `ModalCambiarPassword.css`/`LoginScreen.css` 18 c/u, `Toast.css` 16, y 19 más con ≤12 cada uno — conteo exacto por archivo verificado por `grep`, no estimado). **Sin verificar visualmente en ningún caso** (mismo límite que `U-10`: sin navegador en este entorno) — la corrección de que cada `var()` resuelva al valor hex correcto se validó por inspección de código, no por render real; recomendado que LS confirme visualmente con el SO en modo oscuro antes de dar la Fase 2 por buena en cada archivo migrado |
| **U-13** 🟢 | Descubierto cerrando `ARCH-16` (12 de julio): `HorariosView.jsx` recibe la prop `modoConsulta` (calculada en `HorariosLayout.jsx` como `modoConsulta || !permisos.puedeEditarHorarios`) pero no la usa en ningún lado del cuerpo — a diferencia de `DocentesView`/`MateriasView`, que sí la leen para mostrar su banner de solo-lectura. `TurnoGrid.jsx` tampoco tiene ningún mecanismo de solo-lectura ni de edición in-line (solo expandir celda). No está claro si el "editar horarios" (drag-and-drop) que describe el permiso `puedeEditarHorarios` (`"Arrastrar y colocar bloques, editar in-line"`) nunca se conectó a esta vista, o si vive en otro lado. No se investigó/implementó en `ARCH-16` — es una pregunta de producto, no un hallazgo de linting | `src/components/HorariosView.jsx`, `src/components/TurnoGrid.jsx` | 🔴 Abierto — confirmar si el drag-and-drop de horarios existe en algún otro componente no revisado, o si es una funcionalidad pendiente de conectar |
