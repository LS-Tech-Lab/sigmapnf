...
2. **`U-11`** 🟢 — Reglas responsive de `.qrp-*` viven en `index.css` en
   vez de `QRProyeccion.css` (deuda cosmética, ya documentada en `U-8`)
3. **`U-12`** 🟢 — Sin soporte de `prefers-color-scheme` (modo oscuro) —
   mejora de producto, no defecto
4. **`ARCH-18`** 🟢 — Chunk principal (446 KB / 149 KB gzip) sigue siendo
   el más pesado; pendiente confirmar con `vite-bundle-visualizer` qué
   módulo pesa más antes de invertir tiempo ahí
5. **`ARCH-19`**, **`U-13`** — 2 inconsistencias de comportamiento
   descubiertas incidentalmente cerrando `ARCH-16` (no son hallazgos de
   linting, quedan para revisión de producto)

`ARCH-15`, `ARCH-16`, `ARCH-17`, `SEC-14` y `SEC-15` ✅ cerrados — ver
tablas de Arquitectura y Seguridad abajo.
