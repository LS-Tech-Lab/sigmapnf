...
2. **`ARCH-18`** 🟢 — Chunk principal (446 KB / 149 KB gzip) sigue siendo
   el más pesado; pendiente confirmar con `vite-bundle-visualizer` qué
   módulo pesa más antes de invertir tiempo ahí
3. **`ARCH-19`**, **`U-13`** — 2 inconsistencias de comportamiento
   descubiertas incidentalmente cerrando `ARCH-16` (no son hallazgos de
   linting, quedan para revisión de producto)
4. **`U-12`** 🟡 — Modo oscuro, en progreso por fases — ver detalle abajo

`ARCH-15`, `ARCH-16`, `ARCH-17`, `SEC-14`, `SEC-15` y `U-11` ✅ cerrados —
ver tablas de Arquitectura y Seguridad abajo.
