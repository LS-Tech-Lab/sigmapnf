...
el HEAD real `23628f9`, sin reabrir ningún hallazgo previamente cerrado —
153/153 tests reales, `eslint .` 0 errores/33 warnings, build limpio,
...
| **ARCH-23** ✅ | `DocenteScan/index.jsx` (la pantalla que usan los docentes para marcar entrada/salida por QR) llegó a **525 líneas**, mezclando validación de cédula, búsqueda de docente, armado del registro y el flujo de guardado offline/online en un solo archivo — mismo problema ya corregido 3 veces antes (`AdminQRPanel`, `HistorialView`/`LogsView`/`LoginScreen`, ver `ARCH-11`/`ARCH-13`/`ARCH-18`), nunca aplicado aquí (auditoría QA del 15 de julio) | `src/components/asistencias/DocenteScan/index.jsx` | ✅ Cerrado (15 de julio) — dividido en orquestador (342 líneas: estado, efectos, handlers, llamadas a Supabase) + 2 componentes presentacionales nuevos: `PasoValidacionCedula.jsx` (pasos `formulario`/`confirmar_nuevo`) y `PasoRegistro.jsx` (pasos `confirmar`/`resultado`), mismo patrón 1:1 que `ARCH-13`/`ARCH-18`. `DocenteScan.flow.test.jsx` (ya existente, testea por DOM no por estructura interna) sigue pasando sin modificar — confirma que el refactor no cambió ningún comportamiento observable |
| **ARCH-24** ✅ | El chunk principal (código que se descarga siempre, incluso solo para ver el login) pasó de **74.47 KB a 134.61 KB** (23.56 KB → 38.92 KB gzip) desde que `ARCH-21` lo dejó en ese número — nadie lo volvió a medir tras `ADMIN-3/4/5`, el reverso de `UX-13` y los fixes de `UX-15`/`UX-16`. Puede ser crecimiento legítimo por funcionalidad nueva, pero el KPI que el propio equipo definió como alerta quedó sin revisar (auditoría QA del 15 de julio) | `vite.config.js`, `src/App.jsx` | ✅ Cerrado (15 de julio) — medido con `rollup-plugin-visualizer` (mismo uso puntual, sin comitear, que en `ARCH-21`). **Corrección de premisa**: el hallazgo decía que `AdminModulo` era "el único de los 3 módulos raíz importado de forma estática", dando a entender que `HorariosLayout`/`AsistenciasModulo` ya eran `lazy()` — falso positivo parcial, confirmado por `grep` de `React.lazy`/`Suspense` en `App.jsx`: **ninguno de los 3 lo era**. El grueso del crecimiento real resultó ser código compartido por cualquier ruta (hooks de `useAppData` como `useUpload`/`useAuth`/`useQRSession`, `LoginScreen` pre-auth, `ModalCambiarPassword`), no los shells en sí — así que la ganancia disponible sin tocar esos hooks es acotada. Se convirtieron `AsistenciasModulo` y `AdminModulo` a `lazy()` + `Suspense` (con el `FullScreenSpinner` ya existente como fallback); `HorariosLayout` se deja estático a propósito, mismo criterio que `ResumenView` en `ARCH-10` (es el módulo de entrada por defecto). Resultado: **134.75 KB → 128.76 KB** (38.92 KB → 37.38 KB gzip), ~4.4% de reducción real. Verificado que ningún chunk nuevo quedó precargado por error (`modulepreload` en `dist/index.html` sin cambios) |
| **ARCH-25** ✅ | Tres archivos con lógica real sin ningún test dedicado: `ReporteRango.jsx` (el mismo componente que causó el bug real `UX-15`, ya corregido, pero sigue sin cobertura), y `AdminModulo.jsx`/`ModuleSelector.jsx` (deciden qué módulos ve cada usuario según su rol — un error ahí es, en la práctica, un problema de control de acceso en la interfaz) (auditoría QA del 15 de julio) | `ReporteAsistencias/ReporteRango.jsx`, `app/AdminModulo.jsx`, `components/ModuleSelector.jsx` | ✅ Cerrado (15 de julio) — 18 tests nuevos, mismo patrón que `PestanaUsuarios.integration.test.jsx` (render real con `@testing-library/react`, no solo funciones puras). `ReporteRango.integration.test.jsx` (4): paginación real contra `asistencias_diarias` con cálculo de % de asistencia, y el flujo de borrado ADMIN-2 (éxito + error de la RPC). `AdminModulo.integration.test.jsx` (8): armado de `TABS` según permisos efectivos, tab activa por defecto, cambio de pestaña, gate de `modoConsulta` en Historial, botón "Módulos" condicional. `ModuleSelector.integration.test.jsx` (6): filtrado real de tarjetas por `tieneHorarios`/`tieneQR`/`tieneAdmin`, `onSelectModule` con el id correcto, fallback de nombre de perfil |
...
| **UX-14** ✅ | Descubierto cerrando `ARCH-19` (12 de julio): `HorariosView.jsx` recibe la prop `modoConsulta` (calculada en `HorariosLayout.jsx` como `modoConsulta || !permisos.puedeEditarHorarios`) pero no la usa en ningún lado del cuerpo — a diferencia de `DocentesView`/`MateriasView`, que sí la leen para mostrar su banner de solo-lectura. Confirmado (13 de julio): en ese momento **no existía ninguna edición in-line de horarios en todo el código** — `TurnoGrid.jsx` solo expandía celda para ver detalle, sin drag-and-drop. El permiso `puedeEditarHorarios` (`"Arrastrar y colocar bloques, editar in-line"`, definido en `usuarios/shared.jsx`) no tenía funcionalidad real detrás. Reclasificado (13 de julio) como mejora de producto planeada, pendiente de specs de LS antes de tocar `TurnoGrid.jsx` | `src/components/HorariosView.jsx`, `src/components/TurnoGrid.jsx`, `src/components/ModalEditarClase.jsx` (nuevo), `src/hooks/useAppData/horarioEditing.js` (nuevo), `src/hooks/useAppData/index.js`, `src/app/HorariosLayout.jsx` | ✅ **Cerrado (15 de julio)** — specs de LS: edición por formulario modal (no drag-and-drop), campos día/bloque de hora/aula/docente/materia + eliminar, cada guardado y cada borrado piden confirmación explícita antes de escribir (reusa `openConfirm`/`useConfirmModal.js`, mismo patrón que `backupActions.js`). Implementado `ModalEditarClase.jsx`: al expandir una celda con permiso, aparece un botón "Editar" que abre el formulario; los selects de docente/materia se alimentan de las tablas `docentes`/`materias` (id real, no texto libre), y el bloque de hora se elige entre `BLOQUES_DIURNO`/`BLOQUES_VESPERTINO` (solo reubica el bloque de inicio de 45 min; la duración original de clases que abarcan varios bloques no se edita automáticamente — documentado como límite conocido en el propio formulario). **Hallazgo técnico clave que definió el diseño**: la celda de horario tiene columnas reales `docente_id`/`materia_id` (FK), pero el texto crudo original sigue viviendo en la columna `clase` y se lee *directamente* (sin pasar por el join) en `GlobalSearch.jsx`, `PlanillaImprimibleBase.jsx`, `VistaAusentes.jsx`, `ConflictosView.jsx`, `historialUtils.jsx` y `SeccionesView.jsx`. Para no dejarlas desincronizadas, cada guardado reescribe `clase` en el mismo formato `"<materia>\nProf. <docente>"` que ya reconoce `parseClase()` (estrategia 1, separador de salto de línea) — las 6 pantallas quedan correctas sin tocarlas. Gating de permisos separado del banner (mismo patrón que `DocentesView`/`MateriasView`): `HorariosLayout.jsx` ahora pasa `modoConsulta` crudo (antes iba combinado con `!permisos.puedeEditarHorarios`, la ambigüedad que este mismo hallazgo había señalado) y gatea la capacidad real con `puedeEditar={permisos.puedeEditarHorarios}` / `puedeBorrar={permisos.puedeBorrarHorarios}` (permisos independientes — un rol personalizado puede tener uno sin el otro; el modal oculta "Guardar" si falta `puedeEditarHorarios` y "Eliminar" si falta `puedeBorrarHorarios`). RLS ya exigía ambos permisos a nivel de base de datos desde `SEC-1`/migración `0045` — el gating de UI es una segunda barrera, no la única. Banner de solo-lectura activado en `HorariosView.jsx` (`.hv-modo-banner`, mismos tokens que `.dv-modo-banner`). Nueva acción `horarioEditing.js` (`saveClase`/`deleteClase`) sigue 1:1 el patrón de `nameEditing.js`: toast + `logAudit` + refetch de horarios tras cada operación.

**Agregado el mismo día (15 de julio), a pedido de LS**: el select de docente/materia ahora ofrece "+ Agregar nuevo docente…"/"+ Agregar nueva materia…" para el caso de un docente recién contratado que aún no está en el catálogo. Al elegirlo aparece un input de texto; al guardar, primero se crea el registro en `docentes`/`materias` (`upsert` por `nombre_raw`, mismo patrón que ya usa `nameEditing.js`) para obtener su id real antes de escribir el horario. **Hallazgo que definió el gating de esta parte**: la política RLS de escritura de `docentes`/`materias` (migración `0046`) exige `puedeEditarDocentes`/`puedeEditarMaterias` **o** `puedeImportarExcel` — no `puedeEditarHorarios` — así que las opciones "+ Nuevo" solo se muestran si `permisos.puedeEditarDocentes || permisos.puedeImportarExcel` (materia: `puedeEditarMaterias || puedeImportarExcel`); un usuario con solo `puedeEditarHorarios` puede mover/reasignar clases entre docentes/materias ya existentes, pero no crear catálogo nuevo, evitando así un error de RLS confuso en el toast.

Verificado con `rm -rf node_modules && npm ci`: `eslint .` 0 errores/33 warnings (mismos preexistentes, ninguno en archivos nuevos), 179/179 tests reales (sin tests dedicados nuevos — no hay suite de integración existente para `TurnoGrid`/`HorariosView` que extender en este alcance), `vite build` limpio (chunk principal 128.76 KB → 130.89 KB; `HorariosView` es chunk lazy separado, 14.47 KB → 16.20 KB tras agregar la creación de docente/materia), `npm audit --omit=dev --audit-level=high`: 0 vulnerabilidades. **Sin verificar visualmente en navegador real** (mismo límite ya documentado en `UX-11`/`UX-13`/`UX-16`: este entorno de auditoría no tiene navegador) — recomendado que LS pruebe editar, borrar y crear un docente/materia nuevo desde un bloque real antes de dar por cerrado |
...
| **UX-19** | "Cambiar módulo" era un botón visible en el topbar (`.asm-back-btn`) en Asistencias y Admin, pero en Horarios estaba enterrado como ítem dentro del dropdown de usuario (`UserMenu`) — misma acción de navegación resuelta de dos formas distintas en el mismo repo, sin ningún hallazgo previo que lo cubriera (reportado por LS, 14 de julio) | `src/app/HorariosTopbar.jsx`, `src/app/UserMenu.jsx`, `src/app/AsistenciasModulo.jsx`, `src/app/AdminModulo.jsx`, `src/index.css` | ✅ **Cerrado (14 de julio)** — se unificó hacia el patrón ya usado en Asistencias/Admin: botón visible "← Módulos" en el topbar de los 3 módulos raíz, mismo lugar y misma clase compartida `.topbar-back-btn` (renombrada desde `.asm-back-btn`, que vivía solo en Asistencias pero ya la reutilizaba Admin también — descubierto al hacer el cambio). `UserMenu.jsx` quedó con una sola responsabilidad (cuenta/sesión: cambiar contraseña, cerrar sesión), sin la opción "Cambiar módulo" ni las props `tieneHorarios`/`tieneQR`/`onCambiarModulo` que ya no usa. Verificado: `vite build` limpio, 153/153 tests reales. Llevó a revisar el sidebar de Horarios por la misma inquietud de consistencia — ver `UX-20` |
| **UX-20** | El sidebar de Horarios quedó con solo 5 ítems de navegación tras `ADMIN-3` (que movió "Sistema" al módulo Admin) y se sentía subutilizado — mucho espacio vertical sin aportar información (reportado por LS, 14 de julio). Al revisar se encontró además un bug de código muerto: `HorariosSidebar.jsx` leía `item.hasBadge` para mostrar el conteo de conflictos, pero ningún ítem de `buildNavGroups.js` tenía `hasBadge: true` desde el refactor de `ADMIN-3` — el badge nunca se renderizaba | `src/app/buildNavGroups.js`, `src/app/HorariosSidebar.jsx` | ✅ **Cerrado (14 de julio)** — decisión de diseño: **no** se convirtió el sidebar a topbar-tabs como Asistencias/Admin (carga programa/lapso/admin que esos módulos no tienen, sería rediseño de mayor riesgo, no unificación real). Se probaron dos formas de "llenar" el espacio con datos — un mini-panel de tarjetas (docentes/conflictos) y un conteo junto a cada ítem de nav — y **ambas se descartaron tras revisión de LS** por duplicar información que ya existe en otro lado: el mini-panel repetía "Conflictos activos" de `ResumenView` (vista por defecto del módulo); el conteo por ítem repetía el header de lista que `SeccionesView`/`DocentesView`/`MateriasView` ya muestran al entrar (`{n} secciones`, `{n} docentes`, `{n} materias`). Conclusión: el único cambio real y no redundante fue reactivar `hasBadge: true` en el ítem "Horarios", que corrige un badge de conflictos muerto — ese sí aporta algo que ninguna otra pantalla muestra (aviso visible mientras se está en otra vista). El resto del sidebar se deja como está: 5 ítems bien pueden ser simplemente eso, sin que haga falta rellenarlo de datos ya visibles en otro lado. Verificado: `vite build` limpio, 153/153 tests reales |
| **UX-21** | El estado de conexión ("En línea"/"Sin conexión" + "Últ. sync") solo era visible dentro del dropdown de Administración (`AdminMenu.jsx`), detrás de un clic — a diferencia de la caja de trimestre (`hl-lapso-box`), que es siempre visible en el sidebar sin interacción (LS, 14 de julio, en la misma revisión de `UX-20`). No era información duplicada en ningún otro lado: lo que sí existe en `HorariosTopbar.jsx` (`isSyncing`, `pendientesCount`) es transitorio — solo aparece mientras sincroniza o si hay registros de asistencia offline pendientes, casi siempre 0 en Horarios — no equivale al estado persistente de conexión | `src/app/HorariosSidebar.jsx`, `src/app/AdminMenu.jsx`, `src/index.css` | ✅ **Cerrado (14 de julio)** — mismo patrón que `UX-19` ("Cambiar módulo"): información persistente enterrada en un menú, promovida a elemento siempre visible. Agregada `.hl-status-box` en el sidebar (mismo trato visual que `hl-lapso-box`, con versión ícono-punto en estado colapsado) mostrando `appData.isOffline` y `appData.lastSync`. Quitado el bloque equivalente de `AdminMenu.jsx` para no duplicarlo en el mismo sidebar. Se conservó el conteo de "N registros" en `AdminMenu.jsx` — no es información de conexión, no está duplicada en ningún otro lado, y es más útil ahí (contexto justo antes de "Borrar datos del trimestre") que en la caja persistente. Verificado: `vite build` limpio, 153/153 tests reales |
| **UX-22** | Reportado por LS desde móvil: al abrir el dropdown de Administración del sidebar y tocar el botón "Administración" de nuevo para cerrarlo, no se cerraba. Causa: condición de carrera preexistente entre el listener de "clic afuera" (`mousedown` en `document`, `AdminMenu.jsx`) y el toggle del propio botón — el botón vive fuera del contenedor del dropdown, así que al tocarlo de nuevo el `mousedown` lo cerraba primero vía el listener externo y el `click` inmediatamente después lo reabría (`setAdminOpen(o => !o)` sobre el estado ya actualizado a `false`). Bug preexistente, no introducido por `UX-19`/`UX-20`/`UX-21` — en desktop pasaba desapercibido porque el hábito es cerrar haciendo clic en otro lado, no en el mismo botón | `src/app/AdminMenu.jsx` | ✅ **Cerrado (14 de julio)** — el listener de "clic afuera" ahora ignora explícitamente los clics sobre `.hl-admin-btn` (`e.target.closest(".hl-admin-btn")`), dejando que el propio botón sea la única fuente de verdad para su toggle. Verificado: `vite build` limpio, 153/153 tests reales |
| **UX-23** | Reportado por LS: en el editor de roles (`ModalRol.jsx`), el contador de permisos del administrador mostraba "17/15" — más permisos activos que el total posible. Causa: `PERMISOS_BASE` (`useAuth.js`, fuente real de autorización) define 17 permisos, pero `GRUPOS_PERMISOS`/`TODOS_LOS_PERMISOS` (`usuarios/shared.jsx`, catálogo que alimenta el checklist de la UI) solo tenía 15 — le faltaban `puedeBorrarSesiones` (gatea el borrado en `HistorialSesiones.jsx`/`TabSesiones.jsx`) y `puedeBorrarReportes` (gatea el borrado en `ReporteRango.jsx`). Ambos permisos ya eran funcionales en el código, solo no existían como checkbox en el editor de roles — un rol personalizado no-admin no podía recibir esos permisos desde la UI aunque el código sí los respetaba | `src/components/usuarios/shared.jsx` | ✅ **Cerrado (15 de julio)** — agregadas las 2 entradas faltantes al grupo "Módulo QR" de `GRUPOS_PERMISOS`. Verificado por script que `GRUPOS_PERMISOS` (17 claves) y `PERMISOS_BASE` (17 claves) quedan exactamente sincronizados, sin faltantes en ningún sentido. `vite build` limpio, 153/153 tests reales |
...
- **15 de julio, auditoría QA senior externa (Arq. 90/100, Seg. 96/100,
  UX 87/100):** clonado fresco contra el HEAD real `23628f9`, sin reabrir
  ningún hallazgo previamente cerrado — 153/153 tests reales, `eslint .`
  0 errores/33 warnings, build limpio, `npm audit` (con y sin
  `devDependencies`): 0 vulnerabilidades. Aporta 7 hallazgos nuevos, todos
  marcados 🔴 **pendientes** (sin código tocado en esta pasada, solo
  diagnóstico): `ARCH-23` (`DocenteScan/index.jsx` a 525 líneas, mismo
  patrón ya corregido 3 veces antes sin aplicar aquí), `ARCH-24` (chunk
  principal casi duplicado desde la medición de `ARCH-21`, sin
  reverificar), `ARCH-25` (`ReporteRango.jsx` —el archivo que causó el
  bug real de `UX-15`— y `AdminModulo.jsx`/`ModuleSelector.jsx` sin
  ningún test dedicado), `SEC-23` (primera corrida real de CodeQL/`SEC-20`
  aún sin confirmar por LS), `SEC-24` (CSP sin endpoint de reporte),
  `UX-17`/`UX-18` (cosméticos, baja prioridad — manifest PWA sin variante
  oscura y comentario obsoleto sobre `UX-13` en `ModuleSelector.css`).
  Informe completo: `docs/AUDITORIA_QA_2026-07-14.md`.
- **15 de julio, sesión de implementación de los pendientes de la
  auditoría anterior:** clonado fresco contra HEAD `8637053` (14 commits
  por delante del `23628f9` de la auditoría — trabajo nuevo ya mergeado
  en paralelo, ninguno reabre un hallazgo previo). Cierra `ARCH-23`
  (`DocenteScan/index.jsx` dividido en orquestador + `PasoValidacionCedula.jsx`/
  `PasoRegistro.jsx`), `ARCH-24` (medido con `rollup-plugin-visualizer`;
  corrección de premisa — ningún módulo raíz era `lazy()`, no solo
  `AdminModulo` como decía el hallazgo original; `AsistenciasModulo`/
  `AdminModulo` convertidos a `lazy()`, `HorariosLayout` se deja estático
  a propósito; 134.75 KB → 128.76 KB gzip), `ARCH-25` (18 tests de
  integración nuevos: `ReporteRango`/`AdminModulo`/`ModuleSelector`) y
  `SEC-24` (`api/csp-report.js` nuevo + CSP con `report-to`/`report-uri`,
  8 tests nuevos). No se tocan `SEC-23` (requiere verificación manual de
  LS en GitHub, no es código), `UX-11` (solo necesita más corridas de
  CI), ni `UX-13`/`UX-14`/`UX-17`/`UX-18` (ya resueltos como decisión de
  producto o diferidos sin acción dedicada). 179/179 tests reales
  (153 previos + 26 nuevos), `eslint .` 0 errores/33 warnings (mismos
  preexistentes), build limpio. Nota: en el `git fetch` previo a la
