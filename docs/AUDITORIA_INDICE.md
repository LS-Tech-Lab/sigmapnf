...
- **6 ago, verificación independiente contra HEAD real (`82aaa99`):** clonado fresco, `npm install` + `eslint .` (0 errores, 23 warnings, 2 reales de `exhaustive-deps` en `MateriasView.jsx`/`AdminQRPanel.jsx`, resto cosmético) + `vitest run` (33 archivos, 280/280, confirma cifra del índice) + `vite build` (limpio, PWA 68 entradas precache) contra el código real, no contra lo documentado. Revisión línea por línea de `0068` (SEDE-11–14) y `0059` (ARCH-33, race condition reproducida empíricamente contra Postgres real) confirma que ambos fixes están en el HEAD probado. Único hallazgo nuevo: `UX-32`. Checklist externo reconfirmado como pendiente (no verificable desde sandbox: `pg_cron`/`SEC-21` y rate limiting nativo de Auth/`SEC-7` en el dashboard real de Supabase). Veredicto: apto para producción, 0 hallazgos bloqueantes propios. Mismo día, cierre de `UX-32` a pedido de LS: `mensajeAmigable()` generalizada de un `if` único a una lista `TRADUCCIONES`, agregando cobertura para `duplicate key`/`foreign key` además del caso de `UX-31`, misma filosofía de no inventar traducciones sin match. 2 tests de regresión nuevos + fallback actualizado. Verificación final: 282/282 tests, 0 errores de lint, build limpio.
- **6 ago, bug reportado por LS (`SEDE-15`):** la pestaña "Planilla" de Asistencias QR mostraba los datos de Cabimas (única sede cargada) sin importar la sede activa elegida. Causa raíz: `PlanillaQR.jsx` se autoabastece con su propio fetch a Supabase (no vive dentro de `AppDataContext`) y nunca consumía `useSedeContext`/`sedeActiva`, a diferencia del resto del módulo QR — quedó fuera de las pasadas `SEDE-3`–`14`. 4 queries sin filtrar: `horarios` (planilla y selector de programas), `docentes_con_cedula()`/fallback `docentes`, y `materias`. Corregido con el mismo patrón de `useDataSync.js`/`useNombresCache.js`. Verificación final: 282/282 tests.
