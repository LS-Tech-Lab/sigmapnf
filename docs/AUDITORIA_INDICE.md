...
| **ARCH-7** | Bundle de producción sin dividir por ruta — el chunk principal pesa 514 KB minificado (165 KB comprimido), por encima del umbral que Vite recomienda y advierte en cada build. Hallazgo de la auditoría QA externa del 5 de julio | `vite.config.js`; vistas grandes sin `React.lazy` (`HistorialView`, `LogsView`, `AsistenciasModulo`) | ✅ **Cerrado** (9 de julio) — se aplicó `lazy()` + `Suspense` a `HorariosView`, `SeccionesView`, `DocentesView`, `MateriasView`, `AsistenciasView` y `UploadPreviewModal` en `HorariosLayout.jsx`. **`ResumenView` se dejó a propósito con import estático** — es la vista por defecto (`useState("resumen")` en `App.jsx`), la que ve todo el mundo al iniciar sesión; volverla lazy no reduce el tiempo real de carga y sí añade un salto de red + parpadeo de "Cargando…" a la pantalla más visitada. `UploadPreviewModal` se envolvió con `fallback={null}` (no `LazyFallback`) porque el componente ya retorna `null` cuando está cerrado — un spinner ahí sería una regresión visual nueva, no una mejora. Verificado contra `vite build` real: el chunk principal bajó de **503 KB a 468.49 KB**, quedando por debajo del umbral de 500 KB de Vite (la advertencia `(!) Some chunks are larger than 500 kB` ya no aparece). 153/153 tests siguen pasando |
...
| **U-6** | El bundle sin dividir (`ARCH-7`) también es un problema de experiencia: pantalla en blanco más larga de lo necesario en la primera carga, especialmente en redes móviles. Hallazgo de la auditoría QA externa del 5 de julio | mismo que `ARCH-7` | ✅ **Cerrado** (9 de julio) — mismo fix que `ARCH-7`, ver esa fila para el detalle completo (incluida la decisión de mantener `ResumenView` sin lazy por ser la vista por defecto) |
...
*Décima cuarta pasada (9 de julio de 2026, misma tarde) — implementación de
la Prioridad #1 (`ARCH-7`/`U-6`). Se aplicó `lazy()` + `Suspense` a 6 de las
7 vistas identificadas en la pasada anterior; `ResumenView` se excluyó
deliberadamente por ser la vista por defecto (decisión tomada durante la
implementación, no un olvido — ver la fila de `ARCH-7` para el razonamiento
completo). Verificado contra HEAD real antes de entregar: 153/153 tests,
`vite build` limpio, chunk principal de 503 KB a 468.49 KB, advertencia de
