...
2. **`SEC-15`** 🟢 — Sin política documentada de rotación de
   `SUPABASE_SERVICE_ROLE_KEY`
3. **`U-11`** 🟢 — Reglas responsive de `.qrp-*` viven en `index.css` en
   vez de `QRProyeccion.css` (deuda cosmética, ya documentada en `U-8`)
4. **`U-12`** 🟢 — Sin soporte de `prefers-color-scheme` (modo oscuro) —
   mejora de producto, no defecto
5. **`ARCH-18`** 🟢 — Chunk principal (446 KB / 149 KB gzip) sigue siendo
   el más pesado; pendiente confirmar con `vite-bundle-visualizer` qué
   módulo pesa más antes de invertir tiempo ahí
6. **`ARCH-19`**, **`U-13`** — 2 inconsistencias de comportamiento
   descubiertas incidentalmente cerrando `ARCH-16` (no son hallazgos de
   linting, quedan para revisión de producto)

`ARCH-15`, `ARCH-16`, `ARCH-17` y `SEC-14` ✅ cerrados — ver tablas de
Arquitectura y Seguridad abajo.
...
| **SEC-14** 🟢 | CI corre `npm audit` (vulnerabilidades de dependencias) pero no hay ningún paso de análisis estático (SAST) sobre el código propio — patrones como uso inseguro de `dangerouslySetInnerHTML` no se revisan de forma automática (auditoría QA del 12 de julio, segunda pasada) | `.github/workflows/ci.yml` | 🔴 Abierto — `ARCH-16` ya agregó ESLint y el paso de lint en CI; ahora solo falta sumar `eslint-plugin-security` a la config existente (`eslint.config.mjs`) o un job de CodeQL, ninguno de los dos requiere infraestructura nueva |
