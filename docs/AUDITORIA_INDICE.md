...
2. **`ARCH-25`** 🟢 — `ReporteRango.jsx` (el archivo que causó el bug real
   de `UX-15`) y `AdminModulo.jsx`/`ModuleSelector.jsx` (gating de
   permisos por rol) sin ningún test dedicado — ver detalle abajo
3. **`ARCH-24`** 🟡 — el chunk principal casi se duplicó (74.47 KB → 134.61 KB)
   desde la medición de `ARCH-21`, sin volver a verificarse — ver detalle abajo
4. **`ARCH-23`** 🟡 — `DocenteScan/index.jsx` llegó a 525 líneas mezclando
   responsabilidades, mismo patrón ya corregido 3 veces antes
   (`ARCH-11`/`ARCH-13`/`ARCH-18`) pero nunca aplicado aquí — ver detalle abajo
5. **`SEC-24`** 🟢 — CSP sin endpoint de reporte (`report-to`/`report-uri`)
   — ver detalle abajo
6. **`UX-11`** 🟡 — 24/30 archivos CSS sin `@media`; `9 passed` confirmado
   en un run real de CI (las 3 pantallas × 3 breakpoints); falta solo
   2-3 corridas más estables antes de sacar `continue-on-error`
7. **`UX-14`** 🟡 — Ya no es una pregunta abierta: convertido en mejora de
   producto planeada (edición in-line de horarios en `TurnoGrid.jsx`),
   sin código tocado todavía — ver detalle abajo
8. **`UX-17`** 🟢 / **`UX-18`** 🟢 — cosméticos, sin prioridad real (manifest
   PWA sin variante oscura; comentario obsoleto en `ModuleSelector.css`
   sobre un mecanismo ya revertido) — ver detalle abajo
9. **`UX-13`** ⛔ — Modo oscuro, revertido a pedido de LS (14 de julio) — ver detalle abajo
...
| **SEC-24** 🟢 | La CSP de `vercel.json` es estricta (correcto) pero no tiene endpoint de reporte (`report-uri`/`report-to`) — si algo intenta violar la política en producción, el navegador lo bloquea en silencio y nadie del equipo se entera (auditoría QA del 15 de julio) | `vercel.json` | 🔴 **Pendiente** — agregar `report-to` (+ `report-uri` por compatibilidad) apuntando a un endpoint propio que loguee a `audit_logs`. Cambio aditivo, no toca ninguna regla existente de la CSP |
...
| **ARCH-23** 🟡 | `DocenteScan/index.jsx` (la pantalla que usan los docentes para marcar entrada/salida por QR) llegó a **525 líneas**, mezclando validación de cédula, búsqueda de docente, armado del registro y el flujo de guardado offline/online en un solo archivo — mismo problema ya corregido 3 veces antes (`AdminQRPanel`, `HistorialView`/`LogsView`/`LoginScreen`, ver `ARCH-11`/`ARCH-13`/`ARCH-18`), nunca aplicado aquí (auditoría QA del 15 de julio) | `src/components/asistencias/DocenteScan/index.jsx` | 🔴 **Pendiente** — dividir en orquestador + 2-3 componentes presentacionales (ej. `PasoValidacionCedula.jsx`, `PasoRegistro.jsx`), replicando 1:1 el patrón ya usado en `ARCH-13`/`ARCH-18` |
| **ARCH-24** 🟡 | El chunk principal (código que se descarga siempre, incluso solo para ver el login) pasó de **74.47 KB a 134.61 KB** (23.56 KB → 38.92 KB gzip) desde que `ARCH-21` lo dejó en ese número — nadie lo volvió a medir tras `ADMIN-3/4/5`, el reverso de `UX-13` y los fixes de `UX-15`/`UX-16`. Puede ser crecimiento legítimo por funcionalidad nueva, pero el KPI que el propio equipo definió como alerta quedó sin revisar (auditoría QA del 15 de julio) | `vite.config.js`, `src/App.jsx` (`AdminModulo` es el único de los 3 módulos raíz que se importa de forma estática, no perezosa) | 🔴 **Pendiente** — confirmar con `rollup-plugin-visualizer` (mismo uso puntual, sin comitear, que ya se hizo en `ARCH-21`) qué entró al chunk principal; si es código legítimo de `AdminModulo`, evaluar volverlo `lazy()` igual que sus 3 vistas internas |
| **ARCH-25** 🟢 | Tres archivos con lógica real sin ningún test dedicado: `ReporteRango.jsx` (el mismo componente que causó el bug real `UX-15`, ya corregido, pero sigue sin cobertura), y `AdminModulo.jsx`/`ModuleSelector.jsx` (deciden qué módulos ve cada usuario según su rol — un error ahí es, en la práctica, un problema de control de acceso en la interfaz) (auditoría QA del 15 de julio) | `ReporteAsistencias/ReporteRango.jsx`, `app/AdminModulo.jsx`, `components/ModuleSelector.jsx` | 🔴 **Pendiente** — test de integración por archivo (mismo patrón que `PestanaUsuarios.integration.test.jsx`/`DocenteScan.flow.test.jsx`): para `ReporteRango`, un caso que cubra la paginación con `id` UUID; para `AdminModulo`/`ModuleSelector`, casos que confirmen que un usuario sin permiso admin nunca ve esas rutas |
