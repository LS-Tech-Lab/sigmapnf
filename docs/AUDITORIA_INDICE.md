...
1. **`U-10`** 🟡 — 24/30 archivos CSS sin `@media`; login confirmado en
   verde en CI real, QR scan y selector de módulos agregados (opción C:
   mock de Supabase) pero sin correr aún contra un browser real
...
| **U-10** 🟡 | 24 de los 30 archivos CSS del proyecto no tienen ningún `@media` — la mayoría se apoya en `flex-wrap`/`overflow-x: auto`, lo cual hoy funciona (verificado en `U-8`), pero no hay forma automática de detectar si un cambio futuro rompe eso en pantallas chicas (auditoría QA del 12 de julio, segunda pasada) | 24 archivos `.css` de `src/`, `.github/workflows/ci.yml`, `playwright.config.js`, `tests/visual/` | 🟡 **Ampliado a las 3 pantallas (13 de julio)** — login confirmado en verde en un run real de GitHub Actions: `3 passed (4.3s)` (Ubuntu 24.04, Chromium 149 vía `cdn.playwright.dev`, sin diffs contra las imágenes base). Se agregaron **QR scan** (`tests/visual/qr-scan.spec.js`, ruta pública sin mock — `/scan?token=...` no pasa por ningún guard de auth y sin datos guardados el propio componente resuelve al paso "selector de tipo" de forma síncrona) y **selector de módulos** (`tests/visual/module-selector.spec.js`, requiere sesión — LS eligió la **opción C** entre tres presentadas: mockear el cliente de Supabase a nivel de red del navegador vía `page.route`, en vez de credenciales reales o un proyecto de staging nuevo, por ser la que menos superficie nueva agrega y coherente con el patrón que ya usa `DocenteScan.flow.test.jsx` en Vitest). Detalle completo de la decisión y de cómo está armado el mock (incluida la clave real de `localStorage` de supabase-js, verificada contra el bundle instalado y no adivinada) en `tests/visual/mockSupabase.js` y en la nota narrativa al final de este documento. **No verificado end-to-end**: QR scan y selector de módulos no corrieron todavía contra un browser real (mismo límite de red del entorno de trabajo que ya afectó a `U-10`/`ARCH-16`/`SEC-14`) — sí se confirmó que `playwright test --list` reconoce los 9 tests (3 pantallas × 3 breakpoints) sin errores de config/sintaxis, y que `vitest`/`vite build` siguen sin romperse con los archivos nuevos. **Pendiente antes de cerrar del todo:** generar las imágenes base de QR scan y selector de módulos (mismo procedimiento que login: `npm run test:visual:update`, revisar visualmente, commitear) y confirmarlas en un run real de CI; luego, 2-3 corridas más sin diffs falsos en las 3 pantallas antes de sacar el `continue-on-error: true` |
