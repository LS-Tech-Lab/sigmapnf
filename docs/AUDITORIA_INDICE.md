...
1. **`SEC-14`** 🟢 — Sin SAST/CodeQL sobre código propio en CI (ya no
   depende de `ARCH-16` — el paso de lint que hacía falta ya está)
2. **`U-10`** 🟡 — 24/30 archivos CSS sin `@media`; regresión visual ya
   cubre login, falta QR scan y selector de módulos (requieren sesión)
3. **`SEC-15`** 🟢 — Sin política documentada de rotación de
   `SUPABASE_SERVICE_ROLE_KEY`
4. **`U-11`** 🟢 — Reglas responsive de `.qrp-*` viven en `index.css` en
   vez de `QRProyeccion.css` (deuda cosmética, ya documentada en `U-8`)
5. **`U-12`** 🟢 — Sin soporte de `prefers-color-scheme` (modo oscuro) —
   mejora de producto, no defecto
6. **`ARCH-18`** 🟢 — Chunk principal (446 KB / 149 KB gzip) sigue siendo
   el más pesado; pendiente confirmar con `vite-bundle-visualizer` qué
   módulo pesa más antes de invertir tiempo ahí
7. **`ARCH-19`**, **`U-13`** — 2 inconsistencias de comportamiento
   descubiertas incidentalmente cerrando `ARCH-16` (no son hallazgos de
   linting, quedan para revisión de producto)

`ARCH-15`, `ARCH-16` y `ARCH-17` ✅ cerrados — ver tablas de Arquitectura
y Seguridad abajo.
