...
1. **`U-10`** 🟡 — 24/30 archivos CSS sin `@media`; `9 passed` confirmado
   en un run real de CI (las 3 pantallas × 3 breakpoints); falta solo
   2-3 corridas más estables antes de sacar `continue-on-error`
2. **`U-12`** 🟡 — Modo oscuro, en progreso por fases — ver detalle abajo
3. **`U-13`** 🟡 — Ya no es una pregunta abierta: convertido en mejora de
   producto planeada (edición in-line de horarios en `TurnoGrid.jsx`),
   sin código tocado todavía — ver detalle abajo

`ARCH-15`, `ARCH-16`, `ARCH-17`, `ARCH-18`, `ARCH-19`, `SEC-14`, `SEC-15`
y `U-11` ✅ cerrados — ver tablas de Arquitectura y Seguridad abajo.
...
| **SEC-14** 🟢 | Sin SAST/análisis estático de seguridad sobre el código propio en CI — solo había `npm audit` (dependencias de terceros) y ESLint (estilo, no vulnerabilidades) | `.github/workflows/codeql.yml` (nuevo) | — | ✅ **Cerrado (13 de julio)** — job nuevo y separado de `test-and-build` (mismo criterio que `visual-regression` de `U-10`): corre `github/codeql-action` sobre `javascript-typescript` en cada PR/push a `main` y semanalmente por cron. Sube resultados a la pestaña Security > Code scanning del repo en vez de fallar el check de PR — un hallazgo de CodeQL no debe bloquear un build que está bien, y sin baseline previo un check bloqueante entrenaría a ignorarlo (mismo problema que documentó `U-10`). Permisos acotados a `contents: read` + `security-events: write`, sin tocar `ci.yml` ni `main.yml` existentes. YAML validado (`yaml.safe_load`) antes de commitear; no se pudo correr el job en sí porque requiere GitHub Actions real (el análisis de CodeQL no es replicable en este sandbox) — **pendiente que LS confirme la primera corrida en Actions** y revise los hallazgos iniciales que reporte (pueden incluir falsos positivos a triar, normal en una primera pasada) |
...
| **ARCH-16** 🟡 | El proyecto no tiene ESLint ni Prettier configurados — no hay ningún archivo de lint en la raíz, ni paso de lint en `ci.yml` (auditoría QA del 12 de julio, segunda pasada) | `eslint.config.mjs` (nuevo), `package.json`, `.github/workflows/ci.yml` | ✅ **Cerrado (12 de julio)** — `eslint.config.mjs` (flat config, ESLint 10; extensión `.mjs` en vez de `.js` para que Node no tenga que adivinar el tipo de módulo, sin tocar `"type"` en `package.json`) con solo 3 plugins deliberadamente mínimos: `@eslint/js` recommended, `eslint-plugin-react-hooks` (**solo** `rules-of-hooks` + `exhaustive-deps` en `warn`, no el preset `recommended` completo del plugin — en la v7 instalada ese preset trae ~15 reglas nuevas orientadas al React Compiler que marcaban como error patrones idiomáticos ya auditados del proyecto; adoptarlo entero habría significado reescribir ~15 archivos, fuera de alcance) y `eslint-plugin-react-refresh` (`only-export-components` en `warn`). `no-unused-vars` con `ignoreRestSiblings: true` (patrón real: `const { salt, hash, ...perfil } = entry` en `pinOffline.js`) y `varsIgnorePattern: '^React$'` (import de React sin usar en varios archivos por el runtime automático de JSX). `no-empty` con `allowEmptyCatch: true`. Adaptado al HEAD post-`ARCH-15`/`ARCH-17`/`U-10`: excluye `playwright-report/`, `test-results/`, `blob-report/` (artefactos de Playwright) y da su propio bloque de globals de Node a `tests/visual/**`/`playwright.config.js` (runner y globals distintos de Vitest). 31 errores reales (mismo diagnóstico ya hecho contra un HEAD anterior, re-confirmado 1:1 contra este), todos código muerto o imports sobrantes, corregidos uno por uno sin tocar lógica de negocio: variables/imports sin usar en `LoginScreen.jsx`, `HorariosLayout.jsx`, `buildNavGroups.js`, `DocentesView.jsx`, `TurnoGrid.jsx`, `AdminQRPanel.jsx` (imports ya no usados tras el split de `ARCH-15`), `realtime.js` (3 variables muertas de un refactor previo que dejó `*TimerRef` sin borrar las originales); 3 funciones de ícono SVG huérfanas y un `SectionHeader` huérfano eliminados en `ProgramaLogo.jsx`/`UploadPreviewModal.jsx` (mismo patrón que `ARCH-9`); `no-useless-escape` en `exportPDF.js` resuelto con `eslint-disable`/`enable` alrededor del template literal completo (el escape es defensivo y real, no se tocó su contenido); `no-useless-assignment` en `api/admin-users.js` y `LoginScreen.jsx` resuelto quitando el valor inicial redundante de `callerEsAdmin`/`loginUser`/`loginProfile`, verificado que ningún camino de control los lee antes de la reasignación real. 2 inconsistencias de comportamiento encontradas durante la limpieza **no se tocaron** por ser cambio de comportamiento y no de linting — documentadas como `ARCH-19` y `U-13` abajo. Quedan 33 warnings (`exhaustive-deps`/`only-export-components`), visibles pero no bloqueantes a propósito. `npm run lint` agregado a `package.json` e integrado en `ci.yml` como paso bloqueante del job `test-and-build` (entre instalación y tests) — el job separado `visual-regression` de `U-10` no se tocó. Verificado con `rm -rf node_modules && npm ci` (misma instalación que usa CI): lint 0 errores/33 warnings, 153/153 tests, `vite build` limpio (chunk principal 447.61 KB, idéntico al HEAD sin este fix — la limpieza de código muerto no afectó el tamaño), `npm audit --omit=dev --audit-level=high`: 0 vulnerabilidades |
...
| **U-12** 🟡 | Sin soporte de `prefers-color-scheme` (modo oscuro) — preferencia de producto, no defecto (auditoría QA del 12 de julio, segunda pasada) | tokens `--color-*`/`--brand-*` en `src/index.css`, 30 archivos `.css` de componentes | 🟡 **En progreso por fases (13 de julio)** — alcance real, verificado por `grep` antes de empezar: ~130 colores hex únicos, 526 apariciones, en 30 archivos (más grande de lo que sugería la descripción original). **Fase 1 (completa):** se extendió `:root` con 11 tokens nuevos para los valores más repetidos que no tenían token (`--color-text-muted`, `--color-success-mid/strong/darker/bg-alt/light/text`, `--color-danger-lighter/text`, `--color-role-coord-bg`, `--color-warning-dark`) — mismo criterio que `FE-3`/`FE-5`. Se agregó un bloque redefiniendo todos los tokens de color para modo oscuro, reusando `--navy-950/900/800/700` (paleta ya probada en producción, sidebar) en vez de inventar colores nuevos. ⚠️ **Desactivado a propósito (decisión explícita de LS):** el bloque dark ya no se activa con `prefers-color-scheme` (preferencia del SO) — se cambió a un selector `:root[data-theme="dark"]`, que hoy nada en la app activa. Reactivación pendiente de: (1) terminar la Fase 2 en los 30 archivos, y (2) un botón dentro del sistema para activar/desactivar (no detección automática por SO) — el selector ya deja esto listo, el botón futuro solo alterna `document.documentElement.dataset.theme`, sin tocar CSS de nuevo. **Fase 2 (en progreso) — archivos migrados a `var()`:** `DocenteScan.css` (29, piloto), `LoginScreen.css` (18), `ModuleSelector.css` (rediseñado a pedido explícito de LS), `ModalCambiarPassword.css` (13/18), `LogsView.css` (18/26), `ResumenView.css` (24/34), `HistorialView.css` (25/44), `AdminQRPanel.css` (~60/80 — **BUG REAL encontrado, no relacionado con modo oscuro**: `var(--color-surface)`/`var(--color-border)` apuntaban a tokens que no existen en `:root`, sin fallback — CSS inválido, tarjetas/botones renderizando con fondo transparente y sin borde en vez del fondo blanco/borde gris que era la intención; corregido — ver detalle en el propio archivo), `ReporteAsistencias/index.css` (50/77; quedan literales los bordes con alpha manual de las stat cards — `#2563EB22` etc., hex de 8 dígitos sin token de "color+alpha" equivalente). Quedan **~20 archivos** — el más grande: `UploadPreviewModal.css` (93, conteo previo a que `ARCH-19` retirara el botón "mostrar más" — revisar de nuevo antes de migrarlo). **Excluidos a propósito** de esta migración, revisados y documentados en el propio archivo: `QRProyeccion.css` (21) — pantalla de proyección en el monitor del salón, señalética fija que debe verse siempre oscura sin importar el tema; `Toast.css` (16), `.lv-audit-pre--antes/despues` en `LogsView.css`, y el "grupo B" púrpura del comparador en `HistorialView.css` (`#F5F3FF`/`#8B5CF6`/`#6D28D9`) — todos tarjetas/pares pastel (fondo claro + texto oscuro del mismo tono) cuyo texto no tiene token equivalente; migrar solo el fondo habría reproducido el bug fondo/texto ya corregido en `.sb`/`.app-error-screen`/los toasts de `Pestana*`. También quedan literales, por no tener token exacto y ser de uso puntual: el hue "sky" (celeste — se repite en `LoginScreen.css` y `ResumenView.css`), el esquema "vespertino" en rosa (`ResumenView.css`), y en `HistorialView.css` el badge "activo" (`#DCFCE7`) y los dots de métrica secciones/docentes/materias (emerald/violet/amber-400, tonos vecinos pero no iguales a los tokens existentes). **Bugs reales encontrados y corregidos** (no hipotéticos — el patrón: antes de la Fase 1 estos tokens tenían un solo valor fijo, así que reusarlos "por su valor" en cualquier parte del código era inofensivo; al agregar el bloque dark, cualquier uso fuera de su categoría semántica —texto usado como fondo, fondo usado como texto— se volvió una bomba de tiempo latente que solo se activa cuando se active el modo oscuro): `.sb` (**el sidebar principal de la app**, visible en casi toda la navegación) tenía su fondo en `var(--color-text-primary)` — con el modo oscuro activo se habría vuelto casi blanco, rompiendo toda la navegación; `.app-error-screen` (pantalla de error fatal) tenía el mismo patrón fondo/texto invertido; `.hl-brand-title` y `.app-error-title` usaban un token de *fondo* como color de *texto*, invisible sobre las superficies ya corregidas a invariantes; `.pu-toast`/`.pr-toast` (`PestanaUsuarios.css`/`PestanaRoles.css`) tenían fondo en `var(--color-text-primary)` con texto blanco fijo — invisible en oscuro. Las 5 correcciones usan `--navy-900` (invariante) o el valor literal exacto de hoy donde no aplica un token invariante — cero cambio visual en el estado actual (modo oscuro desactivado), solo dejan de romperse cuando se reactive. **Verificado tras cada archivo/fix:** `vite build` limpio, 153/153 tests, cada `var()` usado confirmado contra la lista real de tokens en `:root` (`comm`). **Pendiente para continuar la Fase 2:** ~26 archivos por migrar + un barrido más amplio del mismo patrón de bug (`grep` encontró ~15 casos adicionales de menor severidad — bordes divisorios con `var(--color-background-tertiary)`, texto muted con `var(--color-border-secondary)` — que se oscurecen/aclaran de forma inconsistente en vez de romperse del todo; no son urgentes pero conviene revisarlos antes de reactivar el modo oscuro). **Sin verificar visualmente en ningún caso** (mismo límite que `U-10`: sin navegador en este entorno) — recomendado que LS confirme visualmente con `document.documentElement.dataset.theme = 'dark'` en la consola del navegador antes de dar cualquier archivo por cerrado |
| **U-13** 🟡 | Descubierto cerrando `ARCH-16` (12 de julio): `HorariosView.jsx` recibe la prop `modoConsulta` (calculada en `HorariosLayout.jsx` como `modoConsulta || !permisos.puedeEditarHorarios`) pero no la usa en ningún lado del cuerpo — a diferencia de `DocentesView`/`MateriasView`, que sí la leen para mostrar su banner de solo-lectura. Confirmado (13 de julio): hoy **no existe ninguna edición in-line de horarios en todo el código** — `TurnoGrid.jsx` solo expande celda para ver detalle, no hay drag-and-drop en ningún lado, ni rastro de que haya existido (sin comentarios, sin TODOs). El permiso `puedeEditarHorarios` (`"Arrastrar y colocar bloques, editar in-line"`, definido en `usuarios/shared.jsx`) no tiene ninguna funcionalidad real detrás | `src/components/HorariosView.jsx`, `src/components/TurnoGrid.jsx` | 🟡 **Convertido en mejora planeada (13 de julio)** — decisión de LS: no era una pregunta para cerrar con un "sí"/"no", es una funcionalidad real pendiente de construir. Queda anotado como **roadmap: implementar edición in-line de horarios (drag-and-drop de bloques) en `TurnoGrid.jsx`**, activando entonces sí el banner de solo-lectura en `HorariosView.jsx` cuando `modoConsulta` sea `true` (mismo patrón que `DocentesView`/`MateriasView`). No se toca código en esta pasada — es trabajo de feature nueva, no un fix; cuando se aborde, conviene tratarlo con el mismo criterio que la serie `ADMIN-N` de la sección "🆕 Funcionalidad nueva" (specs claras de LS antes de tocar `TurnoGrid.jsx`, que hoy es de solo lectura y ya pasa por varias vistas) |
...
- **12 de julio, verificación de compatibilidad entre sesiones paralelas:**
  clonado fresco del HEAD real (`2559898`) para confirmar que el trabajo
  de `ARCH-17` y `U-10` (hecho en otra sesión, en paralelo a `ARCH-16`) no
  chocaba con nada. Reconfirmado sin reabrir ningún hallazgo: 153/153
  tests, `vite build` limpio, `npm audit`: 0 vulnerabilidades. Revisado en
  detalle el diff de `ModalUsuario.jsx` (el más sensible de los 8
  componentes de `ARCH-17`, por tocar roles/permisos de `SEC-10`):
  puramente aditivo, `shape` verificado contra los usos reales. El
  `exclude` de `tests/visual/**` agregado a `vitest.config.js` para `U-10`
  se confirmó necesario y correcto (sin él, Vitest intenta recoger los
  specs de Playwright y falla). Las 3 imágenes base de
  `tests/visual/__screenshots__/` se abrieron y son capturas reales del
  login (no placeholders ni archivos corruptos). Ambas sesiones
  coordinaron correctamente a través de este mismo documento: la sesión
  de `ARCH-17`/`U-10` dejó `ARCH-16` explícitamente marcado como "en
  progreso en otra sesión" y no tocó ningún archivo en conflicto.

- **12 de julio, cierre real de `ARCH-16`:** rehecho contra el HEAD actual
  (`2559898`, ya con `ARCH-15`/`ARCH-17`/`U-10` integrados) — el fix
  anterior de `ARCH-16` nunca había llegado a `main` (se hizo en un
  entorno de trabajo local que no se subió). `eslint.config.mjs` (flat
  config) agregado y hecho bloqueante en `ci.yml`, dentro del job
  `test-and-build` (sin tocar el job `visual-regression` de `U-10`).
  Mismo diagnóstico de 31 errores reales que en el intento anterior
  (código muerto/imports sobrantes, incluyendo 2 imports en
  `AdminQRPanel.jsx` que quedaron sin uso tras el split de `ARCH-15`),
  corregidos uno por uno sin tocar lógica de negocio — detalle completo
  en la tabla "Testing y arquitectura". 2 inconsistencias de
  comportamiento encontradas durante la limpieza (no tocadas, por ser
  cambio de comportamiento y no de linting) documentadas como `ARCH-19` y
  `U-13`. Verificado con `rm -rf node_modules && npm ci` (misma
  instalación que usa CI): lint 0 errores/33 warnings, 153/153 tests,
  `vite build` limpio (chunk principal 447.61 KB, idéntico al HEAD sin
  este fix), `npm audit --omit=dev --audit-level=high`: 0
  vulnerabilidades. `SEC-14` deja de estar bloqueado por este hallazgo.

- **13 de julio, `U-10` — confirmación en CI real:** LS confirmó
  `3 passed (4.3s)` en un run real de GitHub Actions (log crudo revisado
  línea por línea: descarga de Chromium 149 vía `cdn.playwright.dev`,
  `npm run build`, comparación contra las 3 imágenes base del login sin
  diffs). Con esto resuelto el punto (2) que había quedado pendiente en
  la entrega anterior, quedaba pendiente el punto (1): la decisión de
  alcance sobre autenticación para QR scan/selector de módulos.

- **13 de julio, `U-10` — extensión a QR scan y selector de módulos
  (opción C):** LS eligió la **opción C** (mockear el cliente de
  Supabase a nivel de red del navegador) entre las tres presentadas —
  usuario de prueba real contra producción, proyecto de staging aparte,
  o mock — por ser la que menos superficie nueva agrega (sin secrets
  nuevos, sin infraestructura nueva) y coherente con el patrón que ya usa
  `DocenteScan.flow.test.jsx` en Vitest.

  **QR scan resultó no necesitar ningún mock.** Al revisar
  `DocenteScan/index.jsx` antes de escribir el test se encontró que sin
  datos guardados en `localStorage`, el componente resuelve al paso
  "selector de tipo" (Marcar Entrada/Salida) de forma síncrona, sin
  llamar a Supabase — la ruta `/scan` tampoco pasa por ningún guard de
  auth (`src/App.jsx` la resuelve antes que todos los demás). Se agregó
  `tests/visual/qr-scan.spec.js` sin mock alguno, mismo patrón que
  `login.spec.js`.

  **Selector de módulos sí necesitaba el mock completo.** Se creó
  `tests/visual/mockSupabase.js` como única fuente de verdad del shape
  de sesión/perfil falsos, con dos verificaciones hechas contra el código
  real antes de escribir nada (no adivinadas):
  - La clave de `localStorage` que usa supabase-js v2 por defecto
    (`sb-placeholder-auth-token` para `VITE_SUPABASE_URL=
    https://placeholder.supabase.co`) se confirmó leyendo
    `node_modules/@supabase/supabase-js/dist/umd/supabase.js` — la
    fórmula real es `` `sb-${hostname.split('.')[0]}-auth-token` ``. Una
    clave equivocada acá habría hecho que el test cayera de vuelta al
    login sin ningún error visible, silenciosamente.
  - El shape exacto de la consulta de perfil (`.from("user_profiles")
    .select("*, rol_info:roles!user_profiles_rol_fk(...)").eq("id",...)
    .single()`) se confirmó leyendo `useAuth.js`, y los permisos que
    activan cada módulo (`tieneHorarios`/`tieneQR`/`tieneAdmin`) leyendo
    `useModuloActivo.js` — el perfil falso da acceso a exactamente 2
    módulos (Horarios + Asistencias) para que el selector se muestre
    siempre y no caiga en auto-selección de uno solo.

  El mock usa un **catch-all** para `**/rest/v1/**` (array vacío para
  cualquier tabla no listada explícitamente) en vez de enumerar cada
  tabla que `useAppData`/`useQRSession` consultan en segundo plano
  (`asistencias`, `docentes`, `horarios`, `materias`...) — más resiliente
  a futuros cambios de esquema, cualquier tabla nueva cae en el catch-all
  sin romper nada.

  **Bug propio encontrado y corregido antes de entregar:** el primer
  intento de `module-selector.spec.js` verificaba la ausencia del módulo
  "Sistema" con `getByRole('button', { name: /^sistema$/i })` — pero el
  nombre accesible de cada botón incluye también su descripción larga y
  "Entrar", así que el regex anclado nunca iba a matchear, dando
  "0 encontrados" siempre sin importar si el bug real existía (una
  aserción que no puede fallar). Se corrigió verificando por la clase CSS
  real del componente (`.module-card--admin`, ver `ModuleSelector.jsx`)
  en vez de adivinar el texto accesible completo.

  **No verificado end-to-end** (mismo límite de red de siempre en este
  entorno): `playwright test --list` confirma que reconoce los 9 tests
  (3 pantallas × 3 breakpoints) sin errores de config ni sintaxis, y
  `vitest`/`vite build` siguen sin romperse con los archivos nuevos —
  pero ni QR scan ni selector de módulos corrieron contra un browser real
  ni una sola vez. **Pendiente antes de cerrar `U-10` del todo:** generar
  las imágenes base de estas 2 pantallas nuevas (mismo procedimiento que
  login: `npm run test:visual:update` en un entorno con internet,
  revisar visualmente, commitear) y confirmarlas en un run real de CI;
  después, 2-3 corridas más sin diffs falsos en las 3 pantallas antes de
  sacar el `continue-on-error: true`.

- **13 de julio, `U-10` — imágenes base de las 3 pantallas confirmadas:**
  LS generó las 9 imágenes (`npm run test:visual:update`) en GitHub
  Codespaces. Dos problemas de entorno encontrados en el camino, quedan
  anotados por si se repiten:
  - **Popup del puerto reenviado:** al arrancar el webServer de
    Playwright en el 4173, Codespaces reenvía el puerto automáticamente y
    ofrece un link público (`https://<nombre>-4173.app.github.dev`). Ese
    link es solo el aviso de reenvío — no hace falta abrirlo, y de hecho
    no sirve para nada acá: el servidor solo necesita ser alcanzable
    desde el propio contenedor (`localhost:4173`), que es como Playwright
    lo consulta internamente.
  - **`Timeout 60000ms waiting for config.webServer`:** causa real —
    `npm run preview` (que es lo que arranca `playwright.config.js` como
    `webServer`) solo sirve la carpeta `dist/`, no la genera. Sin un
    `npm run build` corrido antes (con `VITE_SUPABASE_URL`/
    `VITE_SUPABASE_ANON_KEY` de placeholder), `dist/` no existe o queda
    vieja, `vite preview` no tiene nada que servir, y Playwright espera
    los 60s completos sin que el puerto responda nunca. En CI esto no
    pasa porque `ci.yml` corre "Build de producción" como paso separado
    antes de "Correr tests visuales" — pero `test:visual:update` en local
    no hace el build por vos, hay que correrlo aparte primero.

  Las 9 imágenes generadas se revisaron visualmente antes de commitear:
  QR scan muestra el selector "Marcar Entrada/Marcar Salida" correcto en
  los 3 breakpoints; selector de módulos muestra "Bienvenido, Prof. Vista
  Previa" con exactamente 2 tarjetas (Horarios + Asistencias) en los 3,
  sin auto-selección ni tarjeta de Sistema de más — confirma que el mock
  de sesión de la opción C (`tests/visual/mockSupabase.js`) funciona de
  punta a punta contra un browser real, no solo contra la config.
  **Pendiente:** confirmar el próximo run de CI en verde (`9 passed`
  esperado) y 2-3 corridas más estables antes de sacar el
  `continue-on-error: true` — con eso, `U-10` queda cerrado del todo.

- **13 de julio, cierre de `ARCH-18`:** instalado temporalmente
  `rollup-plugin-visualizer` (nunca commiteado) para medir el chunk
  principal módulo por módulo en vez de adivinar. El candidato sospechado
  en la auditoría (`@tabler/icons-webfont`) no era el problema real: era
  `xlsx` (SheetJS, ~750 KB sin comprimir), importado de forma estática en
  `useUpload.js` y arrastrado a cada visita de la app vía
  `useAppData/index.js`, se usara o no la carga de Excel. Fix: `xlsx` y
  `excelParser.js` pasan a `import()` dinámico en los 2 puntos reales de
  uso (`leerWorkbookRaw`, `handleFileUpload`), sin cambiar la firma de
  ninguna función exportada de `excelParser.js` — los 29 tests de
  `excelParser.test.js` y los 3 de `useUpload.integration.test.js` no se
  vieron afectados. Chunk principal: 447.61 KB → 74.47 KB (-83%,
  149.86 KB → 23.56 KB gzip). `xlsx` queda en su propio chunk de 500 KB,
  cargado solo cuando alguien sube un archivo. Verificado con
  `rm -rf node_modules && npm ci`: lint 0 errores/33 warnings, 153/153
  tests, build limpio sin warnings (`chunkSizeWarningLimit: 520` agregado
  para el nuevo tamaño esperado del chunk de `xlsx`), `npm audit`: 0
  vulnerabilidades.

- **13 de julio, `ARCH-19` cerrado y `U-13` reclasificado:** LS decidió
  entre las opciones presentadas para cada uno. `ARCH-19`: mantener el
  comportamiento real actual del preview de carga (mostrar siempre todas
  las filas) en vez de arreglar el límite de 200 que nunca se había
  notado ni pedido — se retiró el estado/botón muerto de
  `UploadPreviewModal.jsx` y la clase CSS huérfana que quedaba. `U-13`:
  no es una pregunta para resolver con código en esta pasada — LS la
  convirtió en pedido de mejora (roadmap): implementar de verdad la
  edición in-line de horarios en `TurnoGrid.jsx` que el permiso
  `puedeEditarHorarios` ya describe pero nunca tuvo funcionalidad detrás.
  Cuando se aborde, activar entonces el banner de solo-lectura en
  `HorariosView.jsx` (mismo patrón que `DocentesView`/`MateriasView`).
  Verificado (`ARCH-19`): `vite build` limpio, 153/153 tests, sin tests
  dedicados a `UploadPreviewModal.jsx` que pudieran romperse.
