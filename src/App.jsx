...
function GlobalSearch({ onNavigate }) {
  ...
  const results = useMemo(() => {
    ...
    RAW_DATA.forEach(d => {
      ...
      const key = `${materia}__${docente}`;
      if (!seen.has(key) && (materia.toLowerCase().includes(lo) || docente.toLowerCase().includes(lo))) {
        seen.add(key);
        out.push({ type: docente ? "clase" : "materia", materia, docente, trayecto: d.trayecto, sheet: d.sheet.trim() });
      }
    });
...
export default function App() {
  ...
  const stats = useMemo(() => ({
    ...
    materias: new Set(RAW_DATA.map(d => parseClase(d.clase).materia)).size,
  }), [byDocente]);
...
function DocentesView({ byDocente, conflicts, initialSel, onConsumeNav }) {
  ...
  // Build weekly grid for selected docente
        ...
        <>
          <div style={{ ...S.card, padding:"14px 18px", marginBottom:20, background:"#FFFBEB",
            ...
            <div style={{ fontSize:13, color:"#92400E" }}>
              <strong>Nota:</strong> Un conflicto ocurre cuando el mismo docente aparece asignado a dos grupos distintos en el mismo día y horario. Haz clic en el nombre del docente para ver su horario completo.
            </div>
          </div>
                ...
                <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:12, color:"#9CA3AF", marginBottom:8 }}>
                      Aparece en {c.entries.length} clases simultáneas:
                    </div>
    ...
    <div style={{ padding:20 }}>
        ...
        <div style={{ ...S.card, padding:"16px 20px" }}>
          ...
          {DAYS.map(d => (
            <div key={d} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
              ...
              <div style={{ flex:1, background:"#F3F4F6", borderRadius:4, height:12, overflow:"hidden" }}>
                <div style={{ width:`${(dayCount[d]/maxDay)*100}%`, height:"100%",
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          ...
          {top8.map(([doc, entries], idx) => (
            <div key={doc} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
              ...
              <div style={{ width:100, background:"#F3F4F6", borderRadius:4, height:10, overflow:"hidden" }}>
                <div style={{ width:`${(entries.length/maxLoad)*100}%`, height:"100%",
                               background:"#7C3AED", borderRadius:4 }} />
              </div>
              <span style={{ fontSize:12, width:24, textAlign:"right", color:"#6B7280", fontWeight:600 }}>
                {entries.length}
              </span>
            </div>
          ))}
        ...
        <div style={{ ...S.card, padding:"16px 20px" }}>
          ...
          {topMaterias.map(([mat, cnt], idx) => (
            <div key={mat} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
              ...
              <div style={{ width:100, background:"#F3F4F6", borderRadius:4, height:10, overflow:"hidden" }}>
                <div style={{ width:`${(cnt/maxMat)*100}%`, height:"100%",
        ...
        <div style={{ ...S.card, padding:"16px 20px" }}>
          ...
          {ALL_TURNOS.map(t => {
            const cnt = RAW_DATA.filter(d=>d.turno===t).length;
            const pct = Math.round((cnt/stats.total)*100);
            const colors = { DIURNO:"#2563EB", MATUTINO:"#059669", VESPETINO:"#DC2626", VESPERTINO:"#DB2777" };
            return (
              <div key={t} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                ...
                <div style={{ flex:1, background:"#F3F4F6", borderRadius:4, height:14, overflow:"hidden" }}>
                  <div style={{ width:`${pct}%`, height:"100%",
                                 background:colors[t]||"#888", borderRadius:4 }} />
                </div>
                <span style={{ fontSize:12, color:"#6B7280", fontWeight:600, width:50, textAlign:"right" }}>
                  {cnt} ({pct}%)
                </span>
              </div>
            );
        ...
        <div style={{ ...S.card, padding:"16px 20px" }}>
          ...
          {ALL_TRAYECTOS.map(t => {
            const cnt = [...new Set(RAW_DATA.filter(d=>d.trayecto===t).map(d=>d.sheet.trim()))].length;
            return (
              <div key={t} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                <span style={S.badge(TRAYECTO_BG[t]||"#f3f4f6", TRAYECTO_COLORS[t]||"#555")}>{t}</span>
                <div style={{ flex:1, background:"#F3F4F6", borderRadius:4, height:12, overflow:"hidden" }}>
                ...
                <span style={{ fontSize:12, width:32, textAlign:"right", color:"#6B7280", fontWeight:600 }}>
                  {cnt}
                </span>
              </div>
            );
